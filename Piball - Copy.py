...
rectangle2 = canvas.create_rectangle(325,670,550,690,fill="red")
...
#check if ball is in canvas
    a = canvas.bbox(ball)
    b = canvas.bbox(rectangle)
    c = canvas.bbox(rectangle2)
    d = canvas.bbox(base1)
    e = canvas.bbox(base2)
...
#Ball interacting with the paddles
    ...
    elif a[1] in range(c[1],c[3]) or a[3] in range(c[1],c[3]):
